...
Underneath, every pipeline stage after the first now owns its stdin the way a redirection from an empty file does, so the flag also holds for a command inside a group, subshell, function, loop, conditional, executable script, or nested `bash -c` that a pipe feeds, and is false for a closed fd 0 (`cmd 0<&-`) and inside a bare `bash -c`. The same ownership reaches `if`, `for`, and `case` as pipeline stages, which previously received no stdin at all: `printf 'a\n' | if true; then read x; fi` left `x` empty where bash reads `a`.
