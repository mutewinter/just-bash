 ...
 */
async function resolveDataBody(options, ctx) {
    if (options.dataFile) {
 ...
 */
async function resolveUrlencodeFiles(options, ctx, base) {
    if (options.urlencodeFiles.length === 0)
        return base;
    ...
    for (const entry of options.urlencodeFiles) {
        ...
        parts.push(entry.name ? `${encodeURIComponent(entry.name)}=${encoded}` : encoded);
    }
    return parts.join("&");
}
/**
...
function fetchBodyToStdoutString(body) {
